...
namespace MySql.Data.MySqlClient
{
	internal class MySqlPacket
	{
		...
		public bool IsLastPacket
		{
			get
			{
				return this.buffer.GetBuffer()[0] == 254 && this.Length <= 5;
			}
		}
		public byte[] Buffer
		{
		...
		{
			byte[] array = this.ReadStringAsBytes();
		...
		public byte[] ReadStringAsBytes()
		{
			...
			byte[] array2 = new byte[(long)num - this.buffer.Position];
			Array.Copy(array, (int)this.buffer.Position, array2, 0, (int)((long)num - this.buffer.Position));
