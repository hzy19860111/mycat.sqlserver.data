...
namespace MySql.Data.MySqlClient
{
    internal class StoredProcedure : PreparableStatement
    {
        ...
        internal string GetCacheKey(string spName)
        {
            ...
            string text = "";
            IEnumerator enumerator = this.command.Parameters.GetEnumerator();
            try
            {
                while (enumerator.MoveNext())
                {
                    if (((MySqlParameter)enumerator.Current).Direction == ParameterDirection.ReturnValue)
                    {
                        str = "?=";
                    }
                    else
        ...
        private string CreateOutputSelect(MySqlParameterCollection parms, bool preparing)
        {
            ...
            foreach (MySqlParameter mySqlParameter in parms)
            {
                if (mySqlParameter.Direction != ParameterDirection.Input && ((mySqlParameter.Direction != ParameterDirection.InputOutput && mySqlParameter.Direction != ParameterDirection.Output) || !this.serverProvidingOutputParameters))
                {
                    //"@" + mySqlParameter.BaseName;
                    string text2 = "@_cnet_param_" + mySqlParameter.BaseName;
                    stringBuilder.AppendFormat(CultureInfo.InvariantCulture, "{0}{1}", new object[]
					{
						text,
						text2
            ...
            if (this.command.Connection.Settings.AllowBatch && !preparing)
            {
                return string.Format(";SELECT {0}", stringBuilder.ToString());
            }
            this.outSelect = string.Format("SELECT {0}", stringBuilder.ToString());
            return string.Empty;
        }
        ...
        public override void Close(MySqlDataReader reader)
        {
            ...
            using (MySqlDataReader mySqlDataReader = new MySqlCommand(this.outSelect, this.command.Connection).ExecuteReader(reader.CommandBehavior))
            {
