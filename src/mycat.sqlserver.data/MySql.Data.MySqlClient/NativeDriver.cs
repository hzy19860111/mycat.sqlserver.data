...
namespace MySql.Data.MySqlClient
{
    internal class NativeDriver : IDriver
    {
        ...
        private void StartSSL()
        {
            ...
            if (this.version.isAtLeast(5, 6, 0) && this.isEnterprise)
            {
                sslProtocols |= SslProtocols.Tls12;
            }
            sslStream.AuthenticateAsClient(this.Settings.Server, clientCertificates, sslProtocols, false);
        ...
        public int PrepareStatement(string sql, ref MySqlField[] parameters)
        {
            ...
            byte[] arg_51_0 = this.packet.Buffer;
            ...
            int num = this.packet.ReadInteger(2);
            int num2 = this.packet.ReadInteger(2);
            this.packet.ReadInteger(3);
            if (num2 > 0)
            {
                parameters = this.owner.GetColumns(num2);
        ...
        internal void SetConnectAttrs()
        {
            if ((this.connectionFlags & ClientFlags.CONNECT_ATTRS) != (ClientFlags)0uL)
            {
                ...
                {
                    PropertyInfo expr_34 = properties[i];
                    string text2 = expr_34.Name;
                    object[] customAttributes = expr_34.GetCustomAttributes(typeof(DisplayNameAttribute), false);
                    if (customAttributes.Length != 0)
                    {
                        text2 = (customAttributes[0] as DisplayNameAttribute).DisplayName;
                    }
                    string text3 = (string)expr_34.GetValue(mySqlConnectAttrs, null);
