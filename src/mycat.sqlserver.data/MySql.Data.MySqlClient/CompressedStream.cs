...
namespace MySql.Data.MySqlClient
{
	internal class CompressedStream : Stream
	{
		...
		private MemoryStream CompressCache()
		{
			...
			ZOutputStream expr_2B = new ZOutputStream(memoryStream, -1);
			expr_2B.Write(buffer, 0, (int)this.cache.Length);
			expr_2B.finish();
		...
		{
			byte[] expr_0B = this.cache.GetBuffer();
			byte b = expr_0B[3];
			expr_0B[3] = 0;
			...
			Array.Copy(buffer, 0, buffer, 7, (int)length2);
			buffer[0] = (byte)(length & 255L);
			buffer[1] = (byte)(length >> 8 & 255L);
			buffer[2] = (byte)(length >> 16 & 255L);
			buffer[3] = b;
			buffer[4] = (byte)(num & 255L);
			buffer[5] = (byte)(num >> 8 & 255L);
			buffer[6] = (byte)(num >> 16 & 255L);
			this.baseStream.Write(buffer, 0, num2);
