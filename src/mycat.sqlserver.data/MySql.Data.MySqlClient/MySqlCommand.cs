...
namespace MySql.Data.MySqlClient
{
	...
	public sealed class MySqlCommand : DbCommand, ICloneable, IDisposable
	{
		...
		private bool AddCallStatement(string query)
		{
			...
			if (MySqlCommand.keywords == null)
			{
				MySqlCommand.keywords = new List<string>(Resources.keywords.Replace("\r", "").Split(new char[]
				{
					'\n'
				}));
			}
			return !MySqlCommand.keywords.Contains(text);
		}
		...
		internal string GetCommandTextForBatching()
		{
			if (this.batchableCommandText == null)
			{
				...
				{
					string text = StringUtility.ToUpperInvariant(new MySqlCommand("SELECT @@sql_mode", this.Connection).ExecuteScalar().ToString());
