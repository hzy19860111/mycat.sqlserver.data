...
namespace MySql.Data.MySqlClient
{
	internal class ResultSet
	{
		...
		{
			bool expr_17 = this.driver.FetchDataRow(this.statementId, this.Size);
			if (expr_17)
			{
				this.totalRows++;
			}
		...
		public bool NextRow(CommandBehavior behavior)
		{
			...
			if (this.rowIndex >= 0)
			{
				...
				catch (MySqlException arg_51_0)
				{
					if (arg_51_0.IsQueryAborted)
					{
						this.readDone = true;
					}
					throw;
		...
		private void ReadColumnData(bool outputParms)
		{
			...
			if (outputParms)
			{
				bool arg_7B_0 = this.driver.FetchDataRow(this.statementId, this.fields.Length);
				this.rowIndex = 0;
				if (arg_7B_0)
				{
