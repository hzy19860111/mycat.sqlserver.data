	...
	internal class MySqlConnectAttrs
	{
		...
		public string ProgramName
		{
			get
			{
				...
				try
				{
					result = Path.GetFileName(Environment.CommandLine.Substring(0, Environment.CommandLine.IndexOf("\" ")).Trim(new char[]
					{
						'"'
					}));
		...
		public string OSDetails
		{
			get
			{
				...
				try
				{
					using (ManagementObjectCollection.ManagementObjectEnumerator enumerator = new ManagementObjectSearcher("SELECT * FROM Win32_OperatingSystem").Get().GetEnumerator())
					{
						if (enumerator.MoveNext())
						{
							result = enumerator.Current.GetPropertyValue("Caption").ToString();
						}
					}
				}
				catch (Exception)
