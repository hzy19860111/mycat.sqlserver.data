...
namespace MySql.Data.MySqlClient
{
	public sealed class MySqlConnectionStringBuilder : DbConnectionStringBuilder
	{
		[CompilerGenerated]
		[Serializable]
		private sealed class MySqlConnectionStringBuilderSealedClass
		{
			public static readonly MySqlConnectionStringBuilder.MySqlConnectionStringBuilderSealedClass instance = new MySqlConnectionStringBuilder.MySqlConnectionStringBuilderSealedClass();
		...
		public int? FabricServerMode
		{
			get;
			internal set;
		}
		public int? FabricScope
		{
			get;
			internal set;
		}
		...
		{
			return MySqlConnectionStringBuilder.options.Get(keyword) != null;
		}
		...
		public override bool Equals(object obj)
		{
			...
			if (this.values.Count != mySqlConnectionStringBuilder.values.Count)
			{
				return false;
			}
			foreach (KeyValuePair<string, object> current in this.values)
			{
				if (!mySqlConnectionStringBuilder.values.ContainsKey(current.Key))
				{
					bool result = false;
					return result;
				}
				object obj2 = mySqlConnectionStringBuilder.values[current.Key];
				if (obj2 == null && current.Value != null)
				{
					bool result = false;
					return result;
				}
				if (current.Value == null && obj2 != null)
				{
					bool result = false;
					return result;
				}
