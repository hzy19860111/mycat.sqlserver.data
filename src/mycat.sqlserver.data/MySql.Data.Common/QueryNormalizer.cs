	...
	internal class QueryNormalizer
	{
		...
		private void CollapseValueList(List<Token> tok, ref int pos)
		{
			...
			while (true)
			{
				int num = pos + 1;
				pos = num;
				if (num >= tok.Count || (tok[pos].Type == TokenType.Symbol && tok[pos].Text == ")") || pos == tok.Count - 1)
				{
					list.Add(pos);
					do
					{
					...
					while (num < tok.Count && !tok[pos].IsRealToken);
					if (pos == tok.Count)
					{
						goto IL_A1;
					}
