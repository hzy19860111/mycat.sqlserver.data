	...
	public struct MySqlGeometry : IMySqlValue
	{
		...
		public static bool TryParse(string value, out MySqlGeometry mySqlGeometryValue)
		{
			...
			try
			{
				...
				{
					string[] array2 = ((text != string.Empty) ? text : array[1]).Replace("POINT (", "").Replace("POINT(", "").Replace(")", "").Split(new char[]
					{
						' '
					});
					if (array2.Length > 1)
		...
		{
			MySqlSchemaRow expr_06 = dsTable.AddRow();
			expr_06["TypeName"] = "GEOMETRY";
			expr_06["ProviderDbType"] = MySqlDbType.Geometry;
			expr_06["ColumnSize"] = 25;
			expr_06["CreateFormat"] = "GEOMETRY";
